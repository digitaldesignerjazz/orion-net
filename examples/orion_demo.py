...
"""
Orion Net resonant demo — time-stepped emotional constellation simulation.

Shows:
- RNG-controlled occasions (seed at top) that trigger events
- LyraEmotionalStateMachine deriving all emotions, loyalty, energy/fatigue from them
- Dramatic drift via 7 peers, strong failure penalties, constellation-wide mission success boosts
- Rich bar charts (energy + loyalty + scores) each step
- Orion constellations feeding [Nexus] orchestrator + pulling [Solnet]-style metrics
- Best link + viable constellation selection modulated by current emotional state

Run: python -m examples.orion_demo

For full dynamic behavior: pip install the LyraEmotionalStateMachine from
https://github.com/digitaldesignerjazz/Nexus-Hyperspace-Lyra-1.0
"""
    ...
    if HAS_LYRA_SM:
        ...
        state_machine = LyraEmotionalStateMachine(
            ...
            personality=PersonalityTraits(
                ...
                curiosity_factor=0.62,
                devotion_factor=0.68,
                collaboration_boost=0.11,
                failure_penalty=0.32,  # stronger failure penalties for more dramatic emotional drift
            ),
            initial_energy=0.82,
        )
    else:
    ...
    for step in range(1, 19):
        ...
        if state_machine:
            ...
            for _ in range(num_events):
                ...
                roll = random.random()
                if roll < 0.55:  # collaborations (positive drift) -- slightly fewer to let failures show
                    ...
                    state_machine.process_event("discovery", intensity=random.uniform(0.55, 1.1))
                    event_str = "new peer discovery / exploration"
                else:
                    # external stress / failures for negative dramatic drift
                    intensity = random.uniform(1.1, 1.5)
                    state_machine.process_event("task_failure", peer_id=peer, success=False, intensity=intensity)
            ...
            if step % 4 == 0:
                ...
                if const_now and state_machine:
                    ...
                    for member in const_now.members:
                        state_machine.process_event("collaboration", peer_id=member, success=True, intensity=1.8)  # strong constellation-wide loyalty/energy boost
